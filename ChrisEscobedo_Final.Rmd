...
bay_tract_grocery_with_buffer <- grocery_count_points %>% 
  st_transform(26910) %>% 
  .[bay_tract_buffer,]
  
## create for loop for every row of bay tract buffer
bay_tract_buffer_with_grocery <- grocery_count_points %>% 
  st_transform(26910) %>% 
  .[bay_tract_buffer[396,],] %>% 
  mutate(
    Total = nrow(.)
  ) %>% 
  group_by(Total) %>% 
...
## rbind to some data frame with just census tract buffers to now include count 
