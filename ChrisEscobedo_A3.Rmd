...
I have considered a household to have a higher energy burden if they have an energy burden that is higher than the median energy burden for the entire bay area. I will sum gas and electricity costs for each individual and then calculate the median. I will include the individuals in the high-vulnerability index if their energy burden is greater than the median. 
...
pums_calculations <-
  ...
  mutate(
    ...
    energy_burden = ifelse(
      (as.numeric(HINCP) > 0),
      (net_energy_cost)/ (as.numeric(HINCP)/12) *100,
      100
   ),
...
heat_vuln_pal <- colorNumeric(
  palette = "YlOrRd",
  domain = pums_calculations$total_high_energy_burden_pop
)
```


The following map shows the number of people that have a higher than average energy burden for the Bay Area. Those that are counted in this map represent the population with the highest heat vulnerability.
...
leaflet() %>%
  ...
  addPolygons(
    ...
    fillColor = ~heat_vuln_pal(total_high_energy_burden_pop),
    color = "white",
    opacity = 0.5,
    fillOpacity = 0.8,
    weight = 1,
    label = ~paste0(
      total_high_energy_burden_pop, 
      " people with an above average energy burden"
    ),
  ...
  addLegend(
    data = pums_calculations,
    pal = heat_vuln_pal,
    values = ~total_high_energy_burden_pop,
    title = "Population with an<br>above average<br>energy burden<br>for the Bay Area"
  )
```
