...
download assessor property data
```{r}
temp <- tempfile()
...
join the parcel data with shape
```{r}
sf_parcels <-
  sf_parcels_shape %>% 
  left_join(
...
## remove duplicates from condos, etc.
duplicate_shapes <- 
  sunset_parcels %>% 
  as.data.frame() %>% 
...
maximum zoning details
```{r}
## use projection to make our calculation in sq feet
