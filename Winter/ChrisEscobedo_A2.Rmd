...
Now we will look at the income and racial breakdowns of North Richmond as a whole and for each score.
...
richmond_race <- read_csv("DECENNIALPL2020.csv") %>% 
  .[-1,] %>% 
  select(!c(NAME)) %>%
  pivot_longer(
    ends_with("N"),
...
mapview(richmond_income_race)

richmond_race_total <-
  richmond_income_race %>% 
  group_by(race) %>% 
