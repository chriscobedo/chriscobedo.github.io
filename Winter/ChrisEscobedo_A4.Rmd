...
mapview(vehicles_to_buildings %>% filter(osm_id == 123525465)) + mapview(bh_blocks)
```
...
The dollar amount used to estimate damage in money to each car was calculated using Kelly Blue Book's average price for a new car in 2022, which is $47,000. 
...
bh_vehicle_damage <-
  ...
  mutate(
    damage = vehicles_buildings * (COST OF CAR) * perc_damage
  ) %>% 
