...
vehicles_to_buildings <- bh_bldg %>% 
  ...
  left_join(total_vehicle_count, by = c("GEOID" = "cbg")) %>% 
  select(osm_id, GEOID, block, pop) %>% 
  mutate(
    buildings_ = nrow(vehicles_to_buildings),
  )
  

mapview(vehicles_to_buildings)
```

*Disclaimer: The census data retrieved for population was missing two blocks in the Northwestern area of Belle Haven. Although this area would likely be affected by flood risk, the data may show underepresentation due to this lack of data.* 
