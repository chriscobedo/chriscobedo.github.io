...
# 
# saveRDS(zip, "richmond_south_zip.rds")

zip <- readRDS("richmond_south_zip.rds")
...
# 
# block_groups <- block_groups("CA")
# 
# zip_cbg <- block_groups %>% 
#   st_centroid() %>% 
#   .[zip, ]
#   
# saveRDS(zip_cbg, "zip_cbg.rds")                     

zip_cbg <- readRDS("zip_cbg.rds")
...
Below is a map of the zip code area that will be used for this analysis.
...
travel_time_mode <-
    getCensus(
      ...
      vintage = 2019,
      region = "block group:*",
      regionin = paste0("state:06+county:+county:013"),
      vars = "group(B08134)"
    )
...
Next we will use data from California Air Resources Board (CARB) Emission Factors (EMFAC) model to estimate the carbon emissions from these work trips from or to the South of Richmond. This model includes emissions rates data, year by year.

```{r, warning = FALSE}
emfac <- 
  read_csv("EMFAC2021-no-plug.csv", skip = 8) %>% 
  transmute(
    Year = `Calendar Year`,
...
The annual amount of greenhouse gas emissions for vehicles entering or leaving the South Richmond zip code area from 20139-2019 is displayed below, in metric tonnes of CO2. It does show a slight, but steady decrease in greenhouse gas emissions from vehicles in the region.

```{r, warning=FALSE}
work_trips_richmond_plot <- work_trips_richmond_ghg %>% 
  group_by(Year) %>% 
  summarize(GHG_average = mean(ghg)) %>% 
  ggplot()+
    geom_line(
    aes(
      x = Year,
...
### Plot of Building Emissions

Below is a graph of the total annual amount of KBTU. The total amounts were estimated from the PG&E data, and normalized using population, jobs, and CDDs and HDDs.
...
richmond_cbgs_pop <-
  ...
  filter(block_group %in% zip_cbg$GEOID) %>% 
  group_by(YEAR) %>% 
...
#   })
# 
# saveRDS(richmond_jobs, "richmond_jobs.rds")

richmond_jobs <- readRDS("richmond_jobs.rds")

richmond_jobs_sum <- richmond_jobs %>% 
  group_by(year) %>% 
...
cdd <- read_csv("cdd.csv") %>% 
  mutate(
    YEAR = as.numeric(time %>% substr(1,4))
  ) %>% 
...
dd <- cdd %>% 
  cbind(hdd %>% select(HDD_mean)) %>% 
  select(YEAR, CDD_mean, HDD_mean)
...
richmond_pge_data_normalized <- richmond_pge_data %>% 
  ...
  left_join(richmond_jobs_sum %>% 
              select(YEAR = year, jobs)
  ) %>% 
  left_join(richmond_cbgs_pop) %>% 
  left_join(dd) %>% 
  mutate(
   norm = case_when(
     ENERGYTYPE %in% c("Elec- Commercial") ~ TOTALKBTU/jobs/CDD_mean,
     ENERGYTYPE %in% c("Elec- Residential") ~ TOTALKBTU/pop/CDD_mean,
     ENERGYTYPE %in% c("Gas- Commercial") ~ TOTALKBTU/jobs/HDD_mean,
     ENERGYTYPE %in% c("Gas- Residential") ~ TOTALKBTU/pop/HDD_mean,
    )
  )

building_emissions_plot <- richmond_pge_data_normalized %>% 
  ggplot()+
    geom_line(
    aes(
      x = YEAR,
  ...
  labs(
    x = "Year",
    y = "TOTAL KBTU",
    title = "Total KBTU, normalized by population, jobs, HDDs and CDDs"
  )

building_emissions_plot
```

This plot demonstrates just how disproportionate electricity use is when compared to residential use, as well as gas usage for both residential and commercial use. This might imply that there is an industry that is using a large amount of electricity, specifically per the number of jobs it is producing. This implies an electricity heavy industry in the region that does not create a proportional amount of jobs for its electricity use.
...
full_emissions <- richmond_pge_data_normalized %>% 
  group_by(YEAR) %>% 
  summarize(TOTALTCO2E = mean(norm)) %>% 
  mutate(
    type = "Buildings"
  ) %>% 
  ungroup() %>% 
  rbind(work_trips_richmond_ghg %>% select(YEAR = Year, TOTALTCO2E = ghg) %>% group_by(YEAR) %>% summarize(TOTALTCO2E = mean(TOTALTCO2E)) %>% mutate(type = "Vehicles"))
...
ggplot(
  ...
  geom_bar(
    stat = "identity", aes(fill = type), position = "stack"
  ) + 
  labs(
    x = "Year", y = "tCO2e", title = "South Richmond Annual Emissions by sector, 2013 to 2019"
  ) 
```

The graph clearly shows that the estimated greenhouse gas emissions from vehicles are much higher than that of buildings.
The emissions of buildings have been normalized by population, jobs, and degree days of cooling and heating.

### Increasing Electric Vehicle trend

The graph below shows the emissions by car type and fuel type over the years. The graph shows zero emissions associated with electric vehicles, which is obviously not the whole picture, as it leaves out electricity generation. 

```{r, warning = FALSE}
work_trips_richmond_types <- work_trips_richmond_ghg %>% 
  mutate(
    Category = if_else(Category == "LDA", "Passenger", "light light-duty trucks")
  )

ggplot(
  work_trips_richmond_types,
  aes(
    x = as.factor(Year), 
    y = ghg
  )
  ...
  geom_bar(
    stat = "identity", aes(fill = Fuel_Type), position = "stack"
  ) + 
  labs(
    x = "Year", y = "tCO2e", title = "South Richmond Emissions by fuel Type for Passenger Cars, 2013 to 2019"
  )+
 # coord_flip() +
  facet_grid(. ~ Category)
```
The graph above  shows a gradual decrease in carbon emissions over the years, and also shows that light light-duty trucks have higher total emissions in the region.

The graph below shows the vehicle miles traveled associated with each vehicle type. 
...
ggplot(
  ...
  labs(
    x = "Year", y = "VMT", title = "South Richmond VMT by Vehicle Type, 2013 to 2019"
  ) +
  facet_grid(. ~ Category)
```

Interesting to note is that LDT1 (light light-duty trucks) have significantly less VMT, but more carbon emissions. 

Another interesting trend is that although VMT is not decreasing by much, although it has slightly gone down after 2017, carbon emissions are still decreasing. This shows that although driving has not changed much, fuel type has had a notiecable impact on carbon emissions.

This likeley indicates a shift towards electric vehicles. This is a plan that the city is likely to pursue. Given how difficult is it to change driving patterns, changing the fuel type is a way to decrease carbon emissions immediately. Although, changing car stock comes with its own challenges and time delays. 
