...
expanded_epa_exemption <- epa_exemption %>% 
  ...
  mutate(
    owner_change = if_else(`Assessed Value.x` / `Assessed Value.y` <= 1.02^4, "same owner", "changing owner"),
...
ownership_plot_epa <- combined_epa_exemption %>% 
  ggplot(
    aes(
      x = `Fiscal Year` %>% factor(levels = unique(combined_epa_exemption$`Fiscal Year`)),
      y = `Assessed Value` %>% factor(levels = unique(combined_epa_exemption$`Assessed Value`))
    )
  ) 
