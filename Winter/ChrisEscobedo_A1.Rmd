...
combined_epa_exemption <- epa_exemption %>% 
  ...
  summarize(mean_val = mean(`Assessed Value`))

ggplot(
  combined_epa_exemption,
  aes(
    x = `Fiscal Year` %>% factor(levels = unique(`Fiscal Year`)),
    y = `Assessed Value`, ## %>% factor(levels = unique(`Assessed Value`)),
##    color = tenure %>% factor(levels = unique(`tenure`))
    group = tenure
    )
  ) +
  geom_line()
